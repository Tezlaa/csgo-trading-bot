...
async def sql_start():
    ...
    base.execute('CREATE TABLE IF NOT EXISTS profile(user_id TEXT PRIMARY KEY, name TEXT, balance INT)')
    base.commit()
...
async def create_profile(user_id, username):
    ...
    if not user:
        cur.execute("INSERT INTO profile VALUES(?, ?, ?)", (user_id, username, 15))
        base.commit()
